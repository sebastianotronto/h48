...
void h48map_clear(h48map_t *);
void h48map_destroy(h48map_t *);
uint64_t h48map_lookup(h48map_t *, uint64_t);
...
void run(void) {
	h48map_t map;
	uint64_t n, i, j, capacity, mod, x, y, v;
	kvpair_t kv, *a, *b;
	...
	a = malloc(n * sizeof(kvpair_t));
	b = malloc(n * sizeof(kvpair_t));
	for (i = 0; i < n; i++) {
		x = readl();
		y = readl();
	...
	h48map_create(&map, capacity, mod);
	for (i = 0; i < n; i++)
